...
async def _route_decision(destination: dict, items: list):
    ...
    try:
        ...
        # DUAL items ship from whichever origin is CHEAPER for this customer — i.e.
        # Auckland until Auckland-southbound costs more than Christchurch-northbound
        # (the crossover naturally lands ~Wellington). Pure cost, no geographic gate:
        # scenario A = dual ex-CHCH, scenario B = dual ex-AKL; the selection below picks
        # the lower total (AKL_BIAS defaults 0 = pure cheapest; >0 nudges toward AKL).
        opt = {}
